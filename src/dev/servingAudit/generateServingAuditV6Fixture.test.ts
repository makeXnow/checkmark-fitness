/**
 * Generate verified V6 serving audit fixture (live PARSER + MACROS).
 * Run: npm run audit:v6
 * Quick sample: AUDIT_LIMIT=5 npm run audit:v6
 */
...
import {
  ...
  type ServingAuditV6Row,
} from './servingAuditLib'
import { runServingAuditV6Core } from './servingAuditV6Core'
import type { ServingAuditV6Fixture } from './servingAuditV6Fixture'
import { validateServingAuditV6Rows } from './validateServingAuditV6Rows'
...
const publicFixturePath = join(rootDir, 'public/dev/serving-audit-v6-latest.json')
const publicCsvPath = join(rootDir, 'public/dev/serving-audit-v6-latest.csv')
...
/** Prefer a worker that returns V6 parser shape (quantity + unit). */
async function resolveV6WorkerOrigin(): Promise<string> {
  for (const candidate of DEFAULT_WORKER_CANDIDATES) {
    ...
    try {
      ...
      if (item && typeof item.quantity === 'number' && typeof item.unit === 'string') {
        return origin
      }
    } catch {
      // try next candidate
  ...
  throw new Error(
    'No V6-capable worker found. Start local API (npm run dev:worker) or deploy the worker, then rerun npm run audit:v6.',
  )
}

describe.runIf(SHOULD_RUN)('generate V6 serving audit fixture', () => {
  it(
    'runs live PARSER + MACROS and writes public fixture',
    async () => {
      const WORKER = await resolveV6WorkerOrigin()
      ...
      const rows: ServingAuditV6Row[] = await runServingAuditV6Core(userInputs, customFoods, {
        ...
        callMacroEstimate: async (body) => {
          ...
          return data as import('../../core/api').MacroEstimateApiResult
        },
      })

      const validation = validateServingAuditV6Rows(rows)
      const fixture: ServingAuditV6Fixture = {
        auditVersion: 'v6',
      ...
      writeFileSync(publicCsvPath, `${servingAuditV6RowsToCsv(rows)}\n`, 'utf8')

      console.log(
        `Wrote ${rows.length} rows (${validation.passed}/${validation.total} passed validation) → public/dev/serving-audit-v6-latest.json`,
      )
      ...
      // Require majority pass — full green on first run may be unrealistic for edge API errors
      expect(validation.passed).toBeGreaterThan(0)
      expect(validation.passed / Math.max(validation.total, 1)).toBeGreaterThan(0.85)

      // Spot-check V6 parser shape on first successful row
      ...
      expect(parser).not.toHaveProperty('consumption')
