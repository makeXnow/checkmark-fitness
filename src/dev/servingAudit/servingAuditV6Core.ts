import { buildV3RowFromTrace, type ServingAuditV6Row } from './servingAuditLib'
...
export type ServingAuditV6Deps = {
...
/** Match app feel: several user inputs at once; macro items per input in parallel. */
...
function buildRowFromEstimate(
  userInput: string,
  item: ParsedFoodItem,
  estimate: MacroEstimateApiResult,
  ...
  return buildV3RowFromTrace({
    ...
    computedMultiplier: trace.aiMultiplier,
    rawParserJson,
    rawMacrosJson,
    result: trace.result
      ? {
...
async function processUserInput(
  ...
  deps: ServingAuditV6Deps,
): Promise<ServingAuditV6Row[]> {
  try {
...
export async function runServingAuditV6Core(
  userInputs: string[],
  customFoods: MacroCustomFood[],
  deps: ServingAuditV6Deps,
