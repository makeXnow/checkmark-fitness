...
/** Literal `text-*-500` classes for icons (Tailwind cannot see runtime `bg-` → `text-` swaps). */
const HABIT_ACCENT_TEXT_CLASS: Record<HabitAccentBg, string> = {
...
export function habitAccentTextClass(bgClass: string): string {
  return HABIT_ACCENT_TEXT_CLASS[bgClass as HabitAccentBg] ?? 'text-neutral-400'
}
